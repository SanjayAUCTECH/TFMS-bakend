...
public class OwnersController : BaseApiController
{
    ...
    public async Task<IActionResult> Create([FromBody] CreateOwnerRequest request)
    {
        ...
        if (result.Success)
            await Log(ActivityType.Insert, ActivityModule.Owners, $"Created Owner #{result.Data!.Id}", result.Data!.Id.ToString(), "Owner");
    ...
    public async Task<IActionResult> Update(int id, [FromBody] UpdateOwnerRequest request)
    {
        ...
        if (result.Success)
            await Log(ActivityType.Update, ActivityModule.Owners, $"Updated Owner #{id}", id.ToString(), "Owner");
        return result.Success ? Ok(result) : NotFound(result);
    }
    ...
    public async Task<IActionResult> Delete(int id)
    {
        ...
        if (result.Success)
            await Log(ActivityType.Delete, ActivityModule.Owners, $"Deleted Owner #{id}", id.ToString(), "Owner");
        return result.Success ? Ok(result) : NotFound(result);
    }
}
