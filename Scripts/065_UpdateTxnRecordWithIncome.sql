-- ============================================================
-- 065: sp_UpdateTxnRecord — Incomes table bhi sync hoga
--      Jab payment update ho to Incomes ka matching record bhi update ho
--      Match: Incomes.ContractId = TxnRecords.ContractId
--             AND Incomes.SourceRef = TxnRecords.ContractId (agar linked ho)
-- Date: July 24, 2026
-- ============================================================
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- 5. Revert + re-distribute installments
        ...
        -- ─────────────────────────────────────────────────────────────
        -- 6. ✅ Incomes table UPDATE — matching income record sync karo
        --    Match karo: ContractId = @ContractId
        --                AND SourceRef = @ContractId  (payment linked record)
        --                AND Source = 'Tenant'
        -- ─────────────────────────────────────────────────────────────
        ...
        -- Incomes mai linked income ka Id dhundo (TxnId se match)
        -- Purpose field mai TxnId reference store hai: "Rent received - ..."
        -- SourceRef = ContractId se match karo latest entry jo is TxnId se linked ho
        IF EXISTS (
            SELECT 1 FROM Incomes
            WHERE ContractId = @ContractId
              AND Source = 'Tenant'
              AND Purpose LIKE '%' + ISNULL(@TxnId, '') + '%'
        )
        BEGIN
            -- Exact TxnId match se update
            UPDATE Incomes
            SET Date         = @TxnDate,
                Mode         = ISNULL(NULLIF(@PaymentMode,''), Mode),
                Amount       = @Amount,
                FundPool     = ISNULL(NULLIF(@FundPoolCode,''), FundPool),
                FundPoolName = ISNULL(NULLIF(@FundPoolName,''), FundPoolName),
                Purpose      = 'Rent received - Installment(s): ' + ISNULL(@AppliedInstallments,'') + ' | Contract: ' + @ContractId,
                UpdatedAt    = GETUTCDATE()
            WHERE ContractId = @ContractId
              AND Source = 'Tenant'
              AND Purpose LIKE '%' + ISNULL(@TxnId, '') + '%';
        ...
        ELSE
        BEGIN
            -- Fallback: ContractId wali latest Incomes entry update karo (subquery se latest Id)
            DECLARE @LatestIncomeId INT;
            SELECT @LatestIncomeId = MAX(Id) FROM Incomes
            WHERE ContractId = @ContractId AND Source = 'Tenant';

            IF @LatestIncomeId IS NOT NULL
            BEGIN
                UPDATE Incomes
                SET Date         = @TxnDate,
                    Mode         = ISNULL(NULLIF(@PaymentMode,''), Mode),
...
PRINT '065 - sp_UpdateTxnRecord: Incomes table bhi update hoga jab payment edit ho';
GO
