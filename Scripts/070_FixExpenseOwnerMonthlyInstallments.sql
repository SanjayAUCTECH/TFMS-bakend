-- ============================================================
-- Update Expense SPs to manage OwnerMonthlyContractInstallments
-- When owner payment via expense, update monthly installments too
-- ============================================================
...
BEGIN
    ...
    -- ── Owner payment → OwnerTransactions + OwnerInstallments + OwnerMonthlyContractInstallments ─
    ...
    BEGIN
        ...
        BEGIN
            ...
            -- ✅ Update OwnerInstallments (existing logic)
            ...
            -- ✅ Update OwnerMonthlyContractInstallments (new logic - same pattern)
            DECLARE @RemainingMonthly DECIMAL(18,2) = @Amount;
            ...
            WHILE @@FETCH_STATUS = 0 AND @RemainingMonthly > 0
            BEGIN
                SET @MToApply      = CASE WHEN @RemainingMonthly >= @MInstBal THEN @MInstBal ELSE @RemainingMonthly END;
                ...
                SET @MNewPayStatus = CASE WHEN @MNewPaid >= @MInstAmt THEN 'Paid' WHEN @MNewPaid > 0 THEN 'Partial' ELSE 'Pending' END;
                ...
                SET @RemainingMonthly = @RemainingMonthly - @MToApply;
                FETCH NEXT FROM minst_cur INTO @MInstId, @MInstAmt, @MInstPaid, @MInstBal;
            END;

            CLOSE minst_cur;
            DEALLOCATE minst_cur;
        END
    END
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            -- Step 2A: Re-apply to OwnerInstallments
            ...
            -- Step 2B: Re-apply to OwnerMonthlyContractInstallments
            DECLARE @RemainingMonthly2 DECIMAL(18,2) = @Amount;
            DECLARE @MInstId2    INT, @MInstAmt2 DECIMAL(18,2), @MInstPaid2 DECIMAL(18,2), @MInstBal2 DECIMAL(18,2);
            DECLARE @MToApply2   DECIMAL(18,2), @MNewPaid2 DECIMAL(18,2), @MNewBal2 DECIMAL(18,2), @MNewStatus2 NVARCHAR(20), @MNewPayStatus2 NVARCHAR(20);
            ...
            WHILE @@FETCH_STATUS = 0 AND @RemainingMonthly2 > 0
            BEGIN
                SET @MToApply2      = CASE WHEN @RemainingMonthly2 >= @MInstBal2 THEN @MInstBal2 ELSE @RemainingMonthly2 END;
                ...
                SET @MNewPayStatus2 = CASE WHEN @MNewPaid2 >= @MInstAmt2 THEN 'Paid' WHEN @MNewPaid2 > 0 THEN 'Partial' ELSE 'Pending' END;

                UPDATE OwnerMonthlyContractInstallments
                SET PaidAmount = @MNewPaid2, 
                    Balance = @MNewBal2, 
                    PaidDate = @Date, 
                    Status = @MNewStatus2, 
                    PaymentStatus = @MNewPayStatus2,
                    PaymentMode = @Mode,
                    ExpenseId = @Id,
                    UpdatedAt = GETUTCDATE()
                WHERE Id = @MInstId2;

                SET @RemainingMonthly2 = @RemainingMonthly2 - @MToApply2;
                FETCH NEXT FROM minst_cur2 INTO @MInstId2, @MInstAmt2, @MInstPaid2, @MInstBal2;
            END;

            CLOSE minst_cur2;
            DEALLOCATE minst_cur2;
...
PRINT '✅ Expense SPs updated - now manage OwnerMonthlyContractInstallments';
GO
