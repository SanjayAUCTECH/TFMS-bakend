-- ============================================================
-- 077: Add Tenant payment → TxnRecords (DR) logic
-- When RecipientRole = 'Tenant':
--   CREATE: INSERT TxnRecords with TxnType = 'DR'
--   UPDATE: UPDATE existing TxnRecord or INSERT if role changed
-- ============================================================
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            BEGIN
                ...
                OPEN inst_cur; FETCH NEXT FROM inst_cur INTO @InstId, @InstAmt, @InstPaid, @InstDue;
                ...
                END; CLOSE inst_cur; DEALLOCATE inst_cur;
                ...
                OPEN mcur; FETCH NEXT FROM mcur INTO @MId, @MAmt, @MPaid, @MBal;
                WHILE @@FETCH_STATUS = 0 AND @RemM > 0
                BEGIN
                    SET @MApp   = CASE WHEN @RemM >= @MBal THEN @MBal ELSE @RemM END;
                    SET @MNPaid = @MPaid + @MApp; SET @MNBal = @MAmt - @MNPaid;
                ...
                END; CLOSE mcur; DEALLOCATE mcur;
            END
        END
    END
    ...
    BEGIN
        ...
        DECLARE @ContractId NVARCHAR(MAX) = (SELECT TOP 1 ContractId FROM Contracts WHERE TenantId = @TenantId AND Status = 'Active' ORDER BY CreatedAt DESC);
        ...
        INSERT INTO TxnRecords (
            TxnId, ContractId, Amount, PaidDate, PaymentMode,
            Description, IssuedBy, FundPoolName, TxnType,
            TenantId, TotalAmount, AppliedInstallments, Unallocated, CreatedAt
        ) VALUES (
            @TxnId, ISNULL(@ContractId, ''), @Amount, @Date, @Mode,
            'Payment to Tenant via expense - ' + @ExpenseId,
            '', @FundPoolName, 'DR',
            @TenantId, @Amount, '', 0, GETUTCDATE()
        );
    END
...
BEGIN
    ...
    -- ── Tenant payment → update/insert TxnRecords ─────────────
    IF @RecipientRole = 'Tenant'
    BEGIN
        ...
        ELSE
        BEGIN
            DECLARE @Tid2  INT           = ISNULL(@RecipientId,(SELECT TOP 1 Id FROM Tenants WHERE Name=@RecipientName));
            DECLARE @Cid2  NVARCHAR(MAX) = (SELECT TOP 1 ContractId FROM Contracts WHERE TenantId=@Tid2 AND Status='Active' ORDER BY CreatedAt DESC);
            DECLARE @Txn2  NVARCHAR(MAX) = 'TXN-'+RIGHT('000000'+CAST((SELECT ISNULL(MAX(Id),0)+1 FROM TxnRecords) AS NVARCHAR),6);
            INSERT INTO TxnRecords(TxnId,ContractId,Amount,PaidDate,PaymentMode,Description,IssuedBy,FundPoolName,TxnType,TenantId,TotalAmount,AppliedInstallments,Unallocated,CreatedAt)
            VALUES(@Txn2,ISNULL(@Cid2,''),@Amount,@Date,@Mode,'Payment to Tenant via expense - '+@OldExpenseId,'',@NewFundPoolName,'DR',@Tid2,@Amount,'',0,GETUTCDATE());
        END
    END
    -- If role changed FROM Tenant to something else, remove old DR TxnRecord
...
PRINT '077 - Tenant TxnRecords DR logic added to Expense SPs';
GO
