-- ============================================================
-- 113: Fix sp_RecordPayment — overflow amount auto-distributes
-- Rule:
--   1. If @InstallmentNo=0  → start from first pending (already works)
--   2. If @InstallmentNo>0  → start from that installment
--      If paid > installment amount → overflow goes to NEXT pending
--   3. Amount se zyada aayi to next installment mein jaaye
--   Always amount-based, not installment-id based
-- ============================================================
...
BEGIN
    ...
    -- ── Load pending installments starting from @InstallmentNo ───
    -- If @InstallmentNo=0 → all pending from beginning
    -- If @InstallmentNo>0 → start from that installment (overflow goes to next)
    ...
    WHERE ContractId=@ContractId
      ...
      AND (@InstallmentNo=0 OR InstallmentNo >= @InstallmentNo)  -- ✅ start from or after
    ...
    -- ── Distribute amount across installments in order ────────────
    ...
    BEGIN
        -- Apply as much as needed/available to this installment
...
PRINT '✅ sp_RecordPayment - overflow auto-distributes to next installments';
GO
