...
-- Fix sp_UpdateCamp: handle both PascalCase (from C# repo) and camelCase (from API tests)
...
AS BEGIN
    ...
    IF @PartnersJson IS NOT NULL AND @PartnersJson<>'[]' AND @PartnersJson<>'null'
        ...
        FROM OPENJSON(@PartnersJson) WITH(
            PascalPartnerId INT            '$.PartnerId',
            CamelPartnerId  INT            '$.partnerId',
            PascalShareType NVARCHAR(50)   '$.ShareType',
            CamelShareType  NVARCHAR(50)   '$.shareType',
            PascalShareValue DECIMAL(18,2) '$.ShareValue',
            CamelShareValue  DECIMAL(18,2) '$.shareValue'
        ) j
    ...
    IF @OwnersJson IS NOT NULL AND @OwnersJson<>'[]' AND @OwnersJson<>'null'
        ...
        FROM OPENJSON(@OwnersJson) WITH(
            PascalOwnerId   INT            '$.OwnerId',
            CamelOwnerId    INT            '$.ownerId',
            PascalShareType NVARCHAR(50)   '$.ShareType',
            CamelShareType  NVARCHAR(50)   '$.shareType',
            PascalShareValue DECIMAL(18,2) '$.ShareValue',
            CamelShareValue  DECIMAL(18,2) '$.shareValue'
...
-- Fix sp_CreateCamp: same dual-case handling
...
END
GO
PRINT 'sp_CreateCamp and sp_UpdateCamp FIXED - dual case JSON handling';
GO
