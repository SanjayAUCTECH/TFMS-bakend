...
NOCOUNT ON;
    ...
    WHERE u.IsDeleted = 0
      ...
      AND (@Source  
    IS NULL OR u.Source  = @Source)
      AND (@Status      IS NULL OR u.Status  = @Status)
      AND (@Designation IS NULL OR s.Designation = @Designation)
      AND (@SearchText  IS NULL
           OR u.Name     LIKE '%' + @SearchText + '%'
           O
R u.Username LIKE '%' + @SearchText + '%');

    SELECT
        u.Id, u.UserId, u.Name, u.Username, u.PasswordHash,
        u.Role, u.Source, u.SourceId, u.Contact, u.Email, u.IsAdmin,
        u.LoginAccess, u.Status, u.MenuAccess, u.LastLogin,
        u.
CreatedAt, u.UpdatedAt, u.AddedBy, u.UpdatedBy,
...
ULL OR u.Role    = @Role)
      ...
      AND (@Status      IS NULL OR u.Status  = @Status)
      AND (@Designation IS NULL OR s.Designat
ion = @Designation)
      AND (@SearchText  IS NULL
           OR u.Name     LIKE '%' + @SearchText + '%'
