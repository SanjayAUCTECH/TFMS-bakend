...
-- ══════════════════════════════════════════════════════════════════════════
-- sp_ReceiveSecurityDeposit — @PaymentStatus parameter add kiya
-- ContractRoomsTrns INSERT mein PaymentStatus column set hoga:
--   'Paid'     → normal SD received (default)
--   'Advanced' → advance SD received
-- ══════════════════════════════════════════════════════════════════════════
ALTER PROCEDURE sp_ReceiveSecurityDeposit
    ...
    @Notes         NVARCHAR(MAX)  = '',
    @PaymentStatus NVARCHAR(50)   = 'Paid',   -- ✅ NEW: 'Paid' | 'Advanced'
...
BEGIN
    ...
    -- ── Step 3: Insert TxnRecord ───────────────────────────────────────────
    DECLARE @TxnRecordId INT;

    INSERT INTO TxnRecords (
        ContractId, TxnType, Amount, TxnDate,
        PaymentMode, PaymentModeId, ChequeNumber,
        FundPoolId, FundPoolName,
        Notes, CreatedBy, CreatedAt, UpdatedAt, IsDeleted
    )
    VALUES (
        @ContractId, 'SD-CR', @Amount, @PaidDate,
        @PaymentMode, @PaymentModeId, @ChequeNumber,
        @FundPoolId, @FundPoolName,
        @Notes, @ReceivedBy, GETDATE(), GETDATE(), 0
    );

    SET @TxnRecordId = SCOPE_IDENTITY();

    -- ── Step 4: Insert Income ──────────────────────────────────────────────
    ...
    -- ── Step 5: Update ContractRooms (per-room SecurityPaidAmount) ────────
    --   Distribute @Amount equally across all rooms of this contract
    ...
    -- ── Step 6: Insert ContractRoomsTrns (per-room SD-CR) ─────────────────
    --   ✅ PaymentStatus = @PaymentStatus ('Paid' or 'Advanced')
    ;WITH RoomRanked2 AS (
    ...
    SELECT
        ...
        'SD-CR',
        @TxnRecordId,
        -- Last room gets remainder to avoid rounding mismatch
        CASE WHEN rr.RowNum = @RoomCount
             THEN @Amount - (@PerRoomAmt * (@RoomCount - 1))
             ELSE @PerRoomAmt END,
        CASE WHEN rr.RowNum = @RoomCount
...
PRINT 'sp_ReceiveSecurityDeposit updated: @PaymentStatus parameter added (Paid | Advanced).';
GO
