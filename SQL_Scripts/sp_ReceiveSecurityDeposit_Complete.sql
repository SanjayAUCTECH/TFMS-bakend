...
-- ══════════════════════════════════════════════════════════════════════════
-- sp_ReceiveSecurityDeposit — COMPLETE FIX
-- ✅ @PaymentStatus parameter added
-- ✅ TxnId auto-generation (TXN-XXXXX format)
-- ✅ IncomeId auto-generation (INC-XXXXXX format)
-- ══════════════════════════════════════════════════════════════════════════
...
BEGIN
    ...
    DECLARE @MaxTxnId INT;
    
    -- Get max Id from TxnRecords to generate next TxnId
    SELECT @MaxTxnId = ISNULL(MAX(Id), 0) + 1 FROM TxnRecords;
    
    -- Generate TxnId in format: TXN-00001, TXN-00002, etc.
    SET @NewTxnId = 'TXN-' + RIGHT('00000' + CAST(@MaxTxnId AS NVARCHAR), 5);
    ...
    -- ── Step 5: Generate IncomeId ──────────────────────────────────────────
    DECLARE @NewIncomeId NVARCHAR(50);
    DECLARE @MaxIncomeId INT;
    
    -- Get max Id from Incomes to generate next IncomeId
    SELECT @MaxIncomeId = ISNULL(MAX(Id), 0) + 1 FROM Incomes;
    
    -- Generate IncomeId in format: INC-000001, INC-000002, etc.
    SET @NewIncomeId = 'INC-' + RIGHT('000000' + CAST(@MaxIncomeId AS NVARCHAR), 6);

    -- ── Step 6: Insert Income ──────────────────────────────────────────────
    ...
    INSERT INTO Incomes (
        IncomeId, ContractId, Source, Head, Amount, [Date],
        Mode, FundPool, FundPoolName, TxnRecordId,
        TenantName, CampName,
        Purpose, VoucherNo, SourceRef,
        CreatedAt, UpdatedAt, IsDeleted
    )
    VALUES (
        @NewIncomeId, @ContractId, 'Tenant', 'SECURITY DEPOSIT', @Amount, @PaidDate,
        @PaymentMode, @FundPoolCode, ISNULL(@FundPoolName, ''), @TxnRecordId,
        @TenantName, @CampName,
        'Security Deposit received', '', '',
        GETDATE(), GETDATE(), 0
    );

    -- ── Step 7: Update ContractRooms (per-room SecurityPaidAmount) ────────
    ...
    -- ── Step 8: Insert ContractRoomsTrns (per-room SD-CR) ─────────────────
    ;WITH RoomRanked2 AS (
...
PRINT '✅✅✅ sp_ReceiveSecurityDeposit COMPLETELY FIXED - TxnId + IncomeId auto-generation added!';
GO
